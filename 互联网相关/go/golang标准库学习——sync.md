...
在这些函数签名的首个参数是指针类型，所以在传值的时候需要取地址；那这个参数的类型为什么不是 int、int32、int64而是指针呢？
...
但除了值类型，还有引用类型；在把一个切片作为 Strore 函数的入参传进去时，会拷贝该切片的值，但是该值也是一个指针，该拷贝指针指向的地址与原切片是一样的，所以后续在修改切片时，Value 里面的切片数据也会被修改。
示例：
```go
